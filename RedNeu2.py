...
def definir_distancias():
	city = np.zeros([n, 2])
	for i in range(n):
		city[i] = (round(random(), 2), round(random(), 2))
	return city

n = int(input("Digite o número de cidades: "))
city = definir_distancias()
d = calc_d(city)
...
print("Matriz das ligações entre as cidades:")
print(v)
print('\n')

print("Caminho encontrado pelo algoritmo: ")
...
for col in range(v.shape[1]):
  for row in range(v.shape[0]):
    if v[row][col] == 1:
      if prev_row != -1:
        dist += d[prev_row][row]
        print("{} -> ".format(prev_row + 1), end="")
      prev_row = row
      break
print("{}".format(prev_row + 1))
summation += dist
print("Distancia:", round(dist, 2))
